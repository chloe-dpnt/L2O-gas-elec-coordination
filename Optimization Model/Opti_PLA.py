...
V_gas = 20  # Number of break points used in the linear approximation
...
KI = pd.read_excel(filename, sheet_name="KI")  # Coefficient of the linear approximation of Weymouth
KI = KI.iloc[:, :].to_numpy()  # ndarray(9,12) for each scenario and each pipeline

KO = pd.read_excel(filename, sheet_name="KO")  # Coefficient of the linear approximation of Weymouth
KO = KO.iloc[:, :].to_numpy()  # ndarray(9,12) for each scenario and each pipeline
...
Q = np.linspace(0, 0.3, num=V_gas)
NQ = len(Q)-1

PR = np.linspace(5, 35, num=V_gas)
NPR = len(PR)-1
...
#  WEYMOUTH
h_q = cp.Variable((L_gas, T))
delta_q = {}  # Dictionary for delta_q with keys (l, t, v)
gamma_q = {}  # Dictionary for gamma_q with keys (l, t, v)
# Define the 3D variables as dictionary entries for flows
for l in range(L_gas):
    for t in range(T):
        for v in range(NQ):
            delta_q[(l, t, v)] = cp.Variable(nonneg=True)
            gamma_q[(l, t, v)] = cp.Variable(boolean=True)

h_pr = cp.Variable((N_gas, T))
delta_pr = {}  # Dictionary for delta_pr with keys (n, t, v)
gamma_pr = {}  # Dictionary for gamma_pr with keys (n, t, v)
# Define the 3D variables as dictionary entries for pressures
for n in range(N_gas):
    for t in range(T):
        for v in range(NPR):
            delta_pr[(n, t, v)] = cp.Variable(nonneg=True)
            gamma_pr[(n, t, v)] = cp.Variable(boolean=True)

SlackU = cp.Variable((L_gas, T), nonneg=True)
SlackD = cp.Variable((L_gas, T), nonneg=True)
...
for t in range(T):
    ...
    # Approximation of the flows
    for l in range(L_gas):

        linear_flow = q[l, t] == Q[0] + cp.sum([(Q[v+1] - Q[v]) * delta_q[(l, t, v)] for v in range(NQ)])
        quad_flow = h_q[l, t] == Q[0]**2 + cp.sum([(Q[v+1]**2 - Q[v]**2) * delta_q[(l, t, v)] for v in range(NQ)])
        constraints += [linear_flow, quad_flow]

        for v in range(NQ):
            segment_contrib_q = delta_q[(l, t, v)] <= 1
            active_segment_q = gamma_q[(l, t, v)] <= delta_q[(l, t, v)]
            constraints += [segment_contrib_q, active_segment_q]

        for v in range(1, NQ):
            inactive_segment_q = delta_q[(l, t, v)] <= gamma_q[(l, t, v-1)]
            constraints += [inactive_segment_q]

    # Approximation of pressures
    for n in range(N_gas):

        linear_pressure = pr[n, t] == PR[0] + cp.sum([(PR[v+1] - PR[v]) * delta_pr[(n, t, v)] for v in range(NPR)])
        quad_pressure = h_pr[n, t] == PR[0]**2 + cp.sum([(PR[v+1]**2 - PR[v]**2) * delta_pr[(n, t, v)] for v in range(NPR)])
        constraints += [linear_pressure, quad_pressure]

        for v in range(NPR):
            segment_contrib_pr = delta_pr[(n, t, v)] <= 1
            active_segment_pr = gamma_pr[(n, t, v)] <= delta_pr[(n, t, v)]
            constraints += [segment_contrib_pr, active_segment_pr]

        for v in range(1, NPR):
            inactive_segment_pr = delta_pr[(n, t, v)] <= gamma_pr[(n, t, v-1)]
            constraints += [inactive_segment_pr]

    # Approximation of Weymouth
    for l in range(L_gas):
        if l == 1 or l == 8:  # Skip pipeline if presence of a compressor
            continue
        node1 = pipes[l, 0]
        node2 = pipes[l, 1]
        weymouth = h_q[l, t] == K_w[l] ** 2 * (h_pr[node1 - 1, t] - h_pr[node2 - 1, t])

        constraints += [weymouth]
...
for day in range(3, n_days):
    ...
    g_in_GFPP = cp.sum(GFPPmap @ p) * base / (ConversionFactor * Efficiency[1])
    ...
    GFPP_demand = cp.sum(GFPPmap @ p * base / (ConversionFactor * Efficiency[1]), axis=0).value
