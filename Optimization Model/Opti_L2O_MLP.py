...
model_path = "../Predictor Model/model_MLP.keras"
...
for day in range(3, n_days):
    ...
    x_s = x_s.reshape(1, 144)

    predictions = pred_model.predict(x_s)
    predictions = predictions.reshape(24, 12)  # from shape (288)
    ...
    for l in range(L_gas):
        for t in range(24):
            ...
            if y_rescaled[node1 - 1, t] > y_rescaled[node2 - 1, t]:
                ...
                F00[l, t] = F00[l, t - 1]  # Replace NaN with another value
    ...
    OP_pressure = PR.value
    for m in range(N_gas):
        plt.plot(pression[m, :])
        plt.plot(OP_pressure[m, :])
        plt.title(f"Pressure evolution over time at node {m+1}")
        plt.ylabel('Pressure [bar]')
        plt.xlabel('Time [hours]')
        plt.legend(['Actual', 'Predicted OP'])
        plt.show()
    """
