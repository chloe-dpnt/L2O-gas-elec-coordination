...
# Bounds for the McCormick envelopes
Amin = 40  # pr1+pr2
Amax = 56
Bmin = 0  # pr1-pr2
Bmax = 10
...
# Auxiliary variables
κ = cp.Variable((L_gas, T))
λ = cp.Variable((L_gas, T))
a = cp.Variable((L_gas, T))
b = cp.Variable((L_gas, T))

# Parameters
GasDemandP = cp.Parameter(24, nonneg=True)
WindFactorP = cp.Parameter(24, nonneg=True)
elec_pricesP = cp.Parameter(24, nonneg=True)
...
for t in range(T):
    ...
    for l in range(L_gas):
        ...
        weymouth = cp.SOC(K_w[l] * pr[node1-1, t], cp.hstack([K_w[l] * pr[node2-1, t], q[l, t]]))

        MC1 = a[l, t] == pr[node1-1, t] + pr[node2-1, t]
        MC2 = b[l, t] == pr[node1 - 1, t] - pr[node2 - 1, t]
        MC3 = κ[l, t] >= K_w[l]**2 * λ[l, t]
        MC4 = κ[l, t] >= q[l, t] ** 2
        MC5 = κ[l, t] <= (Qmax + Qmin) * q[l, t] - (Qmax * Qmin)

        MC6 = λ[l, t] >= Amin*b[l, t] + Bmin*a[l, t] - Amin*Bmin
        MC7 = λ[l, t] >= Amax*b[l, t] + Bmax*a[l, t] - Amax*Bmax
        MC8 = λ[l, t] <= Amin*b[l, t] + Bmax*a[l, t] - Amin*Bmax
        MC9 = λ[l, t] <= Amax*b[l, t] + Bmin*a[l, t] - Amax*Bmin

        constraints += [weymouth, MC1, MC2, MC3, MC4, MC5, MC6, MC7, MC8, MC9]
