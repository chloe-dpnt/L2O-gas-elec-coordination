...
from tensorflow.keras.layers import InputLayer, LSTM, Dense, Dropout, Reshape, RepeatVector
...
# Flatten the data to fit the MLP 2D inputs :
x_train_flat = x_train_s.reshape((x_train_s.shape[0], 72 * 2))    # shape: (n_samples, 144)
x_val_flat = x_val_s.reshape((x_val_s.shape[0], 72 * 2))
x_test_flat = x_test_s.reshape((x_test_s.shape[0], 72 * 2))

y_train_flat = y_train_s.reshape((y_train_s.shape[0], 24 * 12))   # shape: (n_samples, 288)
y_val_flat = y_val_s.reshape((y_val_s.shape[0], 24 * 12))
y_test_flat = y_test_s.reshape((y_test_s.shape[0], 24 * 12))
...
model3 = Sequential()
model3.add(Dense(512, activation='relu', input_shape=(72 * 2,)))
model3.add(Dense(256, activation='relu'))
model3.add((Dense(n_timesteps_out * n_features_out, activation='linear')))

model3.summary()

path_checkpoint = "model_checkpoint_MLP1.weights.h5"
...
history = model3.fit(x_train_flat, y_train_flat, batch_size=25, validation_data=(x_val_flat, y_val_flat), epochs=500, callbacks=[cp1, earlystop])
model3.load_weights(path_checkpoint)

model3.save("model_MLP1.keras")
...
def plot_predictions1(model, x, y, training_mean_y, training_std_y, node_number, start=216, end=288):
    ...
    y_pred = predictions.reshape((-1, 24, 12))
    y_pred = y_pred[:, :, node_number]
...
for node_number in range(12):
    plot_predictions1(model3, x_test_flat, y_test_s[:, :, node_number], training_mean_y, training_std_y, node_number,
